...
Il pourrait sembler suffisant, lorsque du texte est saisi dans ce champ, d'en "forcer" la hauteur 
pour qu'elle corresponde à cet attribut.
...
Le clone ne doit pas être un champ texte. Une div, par exemple, assure un bon comportement pour la hauteur : 
le contenu de la div influera bien sur la hauteur de celle-ci.

Dans le CSS on peut remarquer certaines règles destinées à faire correspondre au mieux le volume pris 
par le même contenu dans le champ et son clone (largeur, padding et bordure, font-size, font-family, line-height sont uniformisés).
...
function update_autostretch() {
  ...
  if(clone.offsetHeight >= 200){
   ...
   champ.style.height = clone.offsetHeight + "px";
    ...
    ATTENTION : si le champ contient des retours à la ligne, ceux-ci ne seront pas reproduits dans le contenu du clone,
     ce qui faussera la hauteur obtenue pour le clone
    Mais heureusement, nous avons plus bas une fonction nl2br() pour assurer la conversion des retours à la ligne en balises 
    <br/> et ainsi les retrouver dans le contenu (HTML) du clone.
    
    -> si la hauteur du clone est supérieure ou égale à un seuil minimal (à définir) : 
    donner au champ la même hauteur que celle du clone
...
*/
    ...
    champ.addEventListener("paste", function(){
        setTimeout(update_autostretch,100); //setTimeout car on a besoin de temporiser
        //pour garantir que la valeur du champs est à jour
    });



/* À UTILISER PLUS HAUT : Adaptation de la fonction nl2br() de PHP, servant à convertir, 
pour une chaîne, les retours à la ligne en leur équivalent HTML.
 Sans cela, si l'on cherche par exemple à afficher en HTML le contenu d'un champ texte, 
 les retours à la ligne n'y seront pas pris en compte. */
